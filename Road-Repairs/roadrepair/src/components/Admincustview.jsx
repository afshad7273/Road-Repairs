...
import { getAllCustomersAPI } from "../services/adminServices";
...
const Admincustview = () => {
  ...
  return (
    <div className="flex h-screen w-full bg-gray-100 text-gray-800">
      {/* Sidebar */}
      <div className="w-72 bg-gray-900 text-white p-6 space-y-5 flex flex-col fixed h-screen"> {/* Add fixed here */}
      ...
      {/* Main Content */}
      <div className="flex-1 flex flex-col p-6 ml-72 overflow-y-auto"> {/* Add ml-72 and overflow-y-auto */}
        {/* Navbar */}
        <div className="bg-white p-6 shadow-md flex justify-between items-center rounded-lg">
          ...
          <div className="flex gap-6">
            <span className="text-2xl font-bold bg-gray-900 text-white px-6 py-3 rounded-lg shadow-lg">
              Customers Registered: {data?.length}
            </span>
          </div>
        </div>

        {/* Loading, Error, or Table Container */}
        {isLoading && (
