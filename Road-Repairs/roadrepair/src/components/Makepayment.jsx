...
const PaymentForm = ({ breakdownId }) => {
    ...
    const handlePaymentSubmit = async () => {
        ...
        try {
            ...
            const response = await mutateAsync({breakdownId, token:token.id, paymentMethod});
    ...
    return (
        <div className="max-w-xl w-full p-6 md:p-8 bg-black/70 backdrop-blur-lg rounded-3xl shadow-2xl border border-gray-800">
            ...
            {/* Payment Method Selection - Removed Cash on Delivery */}
            <div className="mb-6">
                <h3 className="text-lg font-medium mb-4 text-gray-300">Payment Method</h3>
                <button
                    onClick={() => { setPaymentMethod("card"); setError(null); }}
                    className={`w-full flex items-center justify-center p-4 border rounded-xl transition-all duration-300 text-base font-medium ${
                        paymentMethod === "card"
            ...
            <button
                ...
                disabled={isLoading || paymentMethod !== 'card' || !stripe || !elements || !!cardError} // Disable if loading, not card, or card issues
            ...
            >
                {isLoading
                    ...
                    : `Pay ₹${breakdownData?.amount.toFixed(2)}`
                }
            </button>
        </div>
    );
